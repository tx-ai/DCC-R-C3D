...
class C3D(nn.Module):
    ...
    def __init__(self):
        ...
        self._initialize_weights()     # 这段好像没有用到
...
class c3d_tdcnn(_TDCNN):
    ...
    def _init_modules(self):
        ...
        self.RCNN_base = nn.Sequential(*list(c3d.features._modules.values())[:-1])  # c3d卷积层的前五层(去掉第五层的MaxPool3d)作为RCNN_base
        # Using fc6
        self.RCNN_top = nn.Sequential(*list(c3d.classifier._modules.values())[:-4])  # c3d全连接层的第一层作为作为RCNN_top
        # Fix the layers before pool2:
        for layer in range(6):
            for p in self.RCNN_base[layer].parameters(): p.requires_grad = False    # 修正一下RCNN_base前两层卷积层的参数

        # not using the last maxpool layer
        self.RCNN_cls_score = nn.Linear(4096, self.n_classes)   # self.n_classes=21  (RCNN_top输出维度是4096,所以这层的输入是4096)
