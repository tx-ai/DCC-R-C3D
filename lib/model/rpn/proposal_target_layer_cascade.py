...
class _ProposalTargetLayer(nn.Module):
    ...
    def __init__(self, nclasses):
        ...
        self.TWIN_NORMALIZE_MEANS = torch.FloatTensor(cfg.TRAIN.TWIN_NORMALIZE_MEANS)   # (0.0, 0.0)
        self.TWIN_NORMALIZE_STDS = torch.FloatTensor(cfg.TRAIN.TWIN_NORMALIZE_STDS)     # (0.1, 0.2)
        self.TWIN_INSIDE_WEIGHTS = torch.FloatTensor(cfg.TRAIN.TWIN_INSIDE_WEIGHTS)     # (1.0, 1.0)

    #               (1,2000,3), (1,20,3)gt_twins前两列代表起止帧,第三列代表标签
    def forward(self, all_rois, gt_twins):
        # all_rois[其中(1,<960,3)<960的部分是前景,第1列全0存21类标签,后两列是前景的起止帧;(960,2000)的部分全0,可能代表背景]
        ...
        gt_twins_append = gt_twins.new(gt_twins.size()).zero_()  # (1,20,3)全0
        gt_twins_append[:, :, 1:3] = gt_twins[:, :, :2]     # (1,20,3)第1列全0,后两列为起止帧
        
        # Include ground-truth twins in the set of candidate rois
        all_rois = torch.cat([all_rois, gt_twins_append], 1)    # (1,2020,3)把20行全0标签和真实起止帧添加在rois2000行末尾
